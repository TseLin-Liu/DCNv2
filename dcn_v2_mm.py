...
class _DCNv2(Function):
    @staticmethod
    def forward(ctx, input, weight, mask, offset, bias, stride, padding):
        ctx.stride = _pair(stride)
        ctx.padding = _pair(padding)
        output = _C.dcn_v2_forward(
            input,
            weight,
            mask,
            offset,
        ...
        ctx.save_for_backward(input, weight, mask, offset, bias)
        return output
    ...
    def backward(ctx, grad_output):
        ...
        return (
            grad_input, grad_weight, grad_mask, grad_offset, grad_bias, None, None,
        )
