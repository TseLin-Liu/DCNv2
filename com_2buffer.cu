        ...
        const int pad_w)
{
    ...
    half* weight_ptr = reinterpret_cast<half*>(weight.to(torch::kHalf).data_ptr<at::Half>());
    half* columns_ptr = reinterpret_cast<half*>(columns.to(torch::kHalf).data_ptr<at::Half>());
    size_t smem = 4 * TILE * (TILE + 8) * sizeof(half);
    
    // im2colcuda<<<Grid0, Block0, smem, stream>>>(
    ...
    // );
    // torch::Tensor columns_fp16 = columns.to(torch::kHalf);
    // half* columns_ptr = reinterpret_cast<half*>(columns_fp16.data_ptr<at::Half>());
